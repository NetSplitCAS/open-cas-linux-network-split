...
__attribute__((format(printf, 2, 3))) int caslog(int log_level, const char *template, ...)
{
	va_list args;
...
__attribute__((format(printf, 2, 3))) int std_printf(int log_level, const char *template, ...)
{
	va_list args;
	va_start(args, template);
...
int check_if_mounted(int cache_id, int core_id)
{
	...
	while ((mstruct = getmntent(mtab)) != NULL)
	{
		...
		if ((NULL != mstruct->mnt_fsname) && (strncmp(mstruct->mnt_fsname, dev_buf, dev_buf_len) == 0))
		{
			if (core_id < 0)
			{
				cas_printf(LOG_ERR,
...
int list_caches(unsigned int list_format)
{
	...
	{
		fprintf(intermediate_file[1], TAG(TREE_BRANCH) "%s,%s,%s,%s,%s,%s\n",
				"core pool", /* type */
				"-",		 /* id */
				"-",
				"-",
	...
	for (i = 0; i < caches_count; ++i)
	{
		...
		fprintf(intermediate_file[1], TAG(TREE_BRANCH) "%s,%u,%s,%s,%s,%s\n",
				"cache",			/* type */
				curr_cache->id,		/* id */
		...
		for (j = 0; j < curr_cache->core_count; ++j)
		{
			...
			fprintf(intermediate_file[1], TAG(TREE_LEAF) "%s,%u,%s,%s,%s,/dev/cas%d-%d\n",
					"core",			/* type */
					curr_core->id,	/* id */
