	...
	int error)
{
	...
	if (context->result)
		result = (context->result != ASYNC_CALL_FINISHED) ? context->result : 0;
	else if (result < 0)
	...
	for (i = 0; i * sizeof(uint64_t) * 8 < OCF_CORE_MAX; i++)
	{
		...
		if (~(bitmap[i] | ignore_mask))
		{
			ret = min((uint64_t)OCF_CORE_MAX,
					  (uint64_t)(i * sizeof(uint64_t) * 8 + _ffz(bitmap[i] | ignore_mask)));
			break;
		}
	}
...
int cache_mngt_cache_check_device(struct kcas_cache_check_device *cmd_info)
{
	...
	if (IS_ERR(bdev))
	{
		return (PTR_ERR(bdev) == -EBUSY) ? -OCF_ERR_NOT_OPEN_EXC : -OCF_ERR_INVAL_VOLUME_TYPE;
	}
...
int cache_mngt_mf_monitor_start(struct kcas_mf_monitor_start *cmd)
{
	...
	{
		result = netcas_mngt_split_monitor_start(core);
	}
	else
	{
